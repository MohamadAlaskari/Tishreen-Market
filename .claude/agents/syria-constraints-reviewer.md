...
6. Hosting/deploy: the compose files (`infra/compose.base.yml` + mode overlay, driven by `tishreen.ps1`) and docs reference a non-US-blocking VPS; backups local; no S3-only assumptions (MinIO/local-disk providers present); images ≤ `storage.max_upload_mb` and resized server-side (bandwidth).
