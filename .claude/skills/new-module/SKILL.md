...
---
Scaffold the backend module **$ARGUMENTS** following `tishreen-handoff/tishreen-handoff/docs/09-architecture.md` §2 and the `spring-modular-monolith` skill.
...
4. Create the application service interface + impl for the first use case listed for this module in `tishreen-handoff/tishreen-handoff/docs/12-build-plan.md`, with `@Transactional`, `@Audited` where the action is mandatory, and a `BusinessException` for each error code in `tishreen-handoff/tishreen-handoff/docs/06 §2` that belongs to the module.
...
6. Create test skeletons named per `tishreen-handoff/tishreen-handoff/docs/13 §2` (`@Disabled("P<n>")` for cases not yet implemented).
7. Run `api/mvnw -f api -q test -Dtest=ArchitectureTest` and report the file list.
