...
3. Name conformance: extract identifiers used in the diff (table names in SQL/JPA, endpoint paths, route paths, permission codes, status strings, settings keys) and grep them in `tishreen-handoff/tishreen-handoff/docs/02`, `06`, `07`, `04`, `tishreen-handoff/tishreen-handoff/CLAUDE.md`; list any that do not appear ("invented name").
4. Workspaces: if `apps/web` or `packages/ui` changed → `pnpm -F web typecheck && pnpm lint && pnpm -F web test`; if `api/` changed → `api/mvnw -f api -q test` (`api/mvnw -f api -q verify` unless `--quick`).
...
7. Print a commit message proposal following `tishreen-handoff/tishreen-handoff/docs/12 §7` (`feat(ordering): …`, `fix(web): …`) and "gate: pass/fail".
