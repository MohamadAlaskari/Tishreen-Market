...
---
# Testing conventions (test names and cases: tishreen-handoff/tishreen-handoff/docs/13-quality.md)
...
- Run: `api/mvnw -f api -q test -Dtest=<Class>`; full `api/mvnw -f api -q verify` (unit + IT + ArchUnit). Paste real output.
...
- API mocked with `msw` handlers per feature (`src/test/handlers/*.ts`) returning DTO shapes from tishreen-handoff/tishreen-handoff/docs/06.
- Run: `pnpm -F web test` / `pnpm -F web test -- <file>`; E2E `pnpm -F web e2e` (Playwright, needs the local stack: `./tishreen.ps1 -Mode dev`).
...
**Definition of done**: tests exist for every rule touched, run green locally, named exactly as in tishreen-handoff/tishreen-handoff/docs/13, no `@Disabled` without a phase tag.
