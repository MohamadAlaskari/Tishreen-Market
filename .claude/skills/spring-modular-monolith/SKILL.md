...
---
# Spring modular monolith conventions (full text: tishreen-handoff/tishreen-handoff/docs/09-architecture.md §2–§7)
...
**Errors**: throw `new BusinessException(ErrorCode.X, Map.of(...))`; `GlobalExceptionHandler` maps to `{code, message(localized), details, correlationId}` with HTTP codes from tishreen-handoff/tishreen-handoff/docs/06 §2; validation → `400` with `details.fields`; never leak stack traces.
