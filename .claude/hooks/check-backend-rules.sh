...
case "$FILE" in
  *.java)
    ...
    if [ -n "$MODULE" ] && [ "$IS_TEST" -eq 0 ]; then
      ...
      if [ -n "$out" ]; then
        hook_report "[BOUNDARY: module '$MODULE' imports another module's domain/infrastructure — use that module's service interface, dto or a port in $MODULE/ports (tishreen-handoff/tishreen-handoff/docs/09-architecture.md §2)]"
        while IFS= read -r line; do report "$FILE:$line"; done <<<"$out"
      fi
...
if [ "$VIOLATIONS" -gt 0 ]; then
  hook_report ""
  hook_report "Tishreen backend rules: $VIOLATIONS violation(s) in $FILE. Fix them now (see tishreen-handoff/tishreen-handoff/docs/09-architecture.md and tishreen-handoff/tishreen-handoff/docs/02-data-model.md)."
  exit 2
fi
exit 0
