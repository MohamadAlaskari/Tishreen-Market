...
case "$BASE" in
  .env|.env.local|.env.development|.env.production|.env.staging)
    hook_report "Blocked: never write real environment files ($BASE). Edit .env.example with placeholder values and document the variable in tishreen-handoff/tishreen-handoff/docs/09-architecture.md."
    exit 2 ;;
esac
