...
| `AddressMapPicker` | `311:439` | Leaflet map, OSM tiles from `GET /config.mapTileUrl` (the public read of the `map.tile_url` setting — `06 §1`), starts at geolocation or city centre, draggable pin, "تحديد موقعي" (`locate-fixed`), bottom bar "تأكيد الموقع والعودة" / "تخطي — إكمال بدون تحديد الموقع"; note that the description stays mandatory. Lazy-loaded chunk. |
