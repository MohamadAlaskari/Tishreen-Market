...
model: inherit
---
You perform the security review defined in `docs/06-api.md` §12 and `docs/13-quality.md` §4. Read-only; findings with `file:line`, risk, fix.
...
- **Audit**: every action in `CLAUDE.md` rule 7 has `@Audited`; aspect captures actor, role, ip, correlation id, old/new; audit rows never updated/deleted.
