...
---
# Tishreen domain in one page (details: docs/00, 01, 05)
...
- Settings groups `BUSINESS` (admin) / `TECHNICAL` (IT) / `PROVIDERS` (IT). Keys: `store.* orders.* delivery.max_attempts loyalty.* otp.* bot.match_threshold auth.* storage.* backup.* logs.* map.tile_url notification.provider storage.provider payment.provider chatbot.provider` (values in docs/04).
