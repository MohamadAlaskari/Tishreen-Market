...
2. Delegate to the `design-token-guardian` subagent (pass paths and node id). If a node id is given, have it fetch `get_metadata` + `get_screenshot` and compare against the spec row in `docs/07`.
3. Verify the three presets: render-critical components must not reference a preset; `data-theme` + `.dark` both covered; fonts `font-display|label|mono` used as in `docs/08 §5`.
